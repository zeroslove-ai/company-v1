...
export function createFrontendApp({ documentRef = globalThis.document, storage = globalThis.localStorage, api = createApiClient(), locationSearch = globalThis.location?.search ?? '' } = {}) {
  ...
  const gameId = resolveGameId(locationSearch); let context = null, latestResult = {}, busy = false;
  ...
  function render() {
    ...
    renderChoices(elements.choices, latestResult.choices?.length ? latestResult.choices : contextChoices(context), { busy, onChoose: startNewAction });
  }
  const setBusy = value => { busy = value; if (elements.input) elements.input.disabled = value; if (elements.submit) elements.submit.disabled = value; render(); };
  ...
  const coordinator = createTurnCoordinator({
    ...
    onCommitted: () => { clearCurrentTurn(); showStatus('턴이 완료되었습니다.'); }
  });
  async function checkRecovery() {
    const pending = loadPending(storage, gameId);
    if (!pending) { if (elements.recovery) elements.recovery.hidden = true; return; }
    try {
      const data = await api.actionStatus({ game_id: gameId, action_id: pending.action_id }); const step = recoveryFor(data);
      if (step === 'complete') { await coordinator.runRecovery(pending, step); return; }
