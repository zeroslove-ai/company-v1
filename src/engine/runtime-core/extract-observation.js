...
function actorChangePath(actorId, path, npcIds) {
  ...
  if (id === 'player') {
    ...
    return (path.startsWith(playerPhysicalPrefix) && (new Set(['position_label', 'posture']).has(path.slice(playerPhysicalPrefix.length)) || /^clothing\.(uniform_top|uniform_bottom|underwear_top|underwear_bottom)$/.test(path.slice(playerPhysicalPrefix.length))))
  ...
  return suffix === 'position_label' || suffix === 'posture' || /^clothing\.(uniform_top|uniform_bottom|underwear_top|underwear_bottom)$/.test(suffix);
}
