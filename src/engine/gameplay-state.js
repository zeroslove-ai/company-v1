...
export function normalizeGameplayExtractEnvelope(value, { parsedStory = {} } = {}) {
  ...
  return {
    ...
    mind_monitor: normalizedMonitor.mind_monitor,
    legacy_mind_monitor_text: normalizedMonitor.legacy_text,
    choices: parserHasChoices ? storyChoices : choices(value.choices),
    dialogue_lines: normalDialogueLines(value.dialogue_lines),
    npcs_present: choices(value.npcs_present),
    action_target_id: identity(value.action_target_id),
    focal_character_id: identity(value.focal_character_id),
    last_speaker_id: identity(value.last_speaker_id),
    image_character_id: identity(value.image_character_id),
...
export function hydrateGameplayState(save, master = {}) {
  ...
  for (const character of characters) {
    ...
    if (!id) continue;
    for (const [mapName, initialKey] of [
      ['npc_stats', 'initial_stats'], ['npc_relationship_state', 'initial_relationship_state'],
      ['npc_emotion', 'initial_emotion'], ['npc_scene_state', 'initial_scene_state']
    ]) {
      next[mapName] = object(next[mapName]) ? next[mapName] : {};
      if (!(id in next[mapName]) && object(character[initialKey])) next[mapName][id] = clone(character[initialKey]);
    }
  }
  return next;
}
