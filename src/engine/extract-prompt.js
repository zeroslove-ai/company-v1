...
const SYSTEM_INSTRUCTIONS = [
  ...
  'If a narrow projection has no exact observed change, omit it. The safe minimal observation for ordinary dialogue is empty projections, block_observations containing facts only for exact observed facts, no semantic event/relation taxonomy, scene_observation with final_present_npc_ids:null, and the remaining structural defaults.',
  'Exact evidence contract: evidence is a top-level sibling of player_observation and npc_observations. Never put an evidence key inside a player or NPC object; npc_observations.<npc_id> contains only the listed domain objects. Omit evidence.changed when there is no changed field instead of returning an empty changed array or empty quote. Clothing uses evidence.clothing.<actor_id>={quote,character_id}; other fields use evidence.changed {changed:[path],quote}; scene evidence uses {kind,character_id or location_id,quote}, where kind is presence, entrance, exit, or scene. Entrance/exit evidence must include the registered character id and an exact contiguous Story quote. kind:"scene" requires non-null scene_observation.scene_id; otherwise omit it. Copy quotes verbatim from story_text; never compose a quote from inferred facts or any input outside story_text. If the exact sentence is not present in story_text, omit the scene evidence. In a multi-NPC scene, an NPC physical/clothing quote must include the actor name; for pronouns, include the contiguous preceding named clause. Scene evidence is only for directly shown facts. Locations must be registered.',
  'Illustrative physical shape (not mandatory output): {"npc_observations":{"heroine2":{"physical":{"position_label":"회의실 테이블 옆","clothing":{"underwear_bottom":"removed"}}}},"evidence":{"clothing":{"heroine2":{"character_id":"heroine2","quote":"named exact Story substring"}},"physical_change":{"changed":["npc_scene_state.heroine2.clothing.underwear_bottom"],"quote":"same exact substring"}}}. Use position_label, never position/label; posture must use only canonical tokens "sitting" or "standing" when clear; do not translate them into free-text tokens, and omit posture when uncertain. Clothing slots are uniform_top, uniform_bottom, underwear_top, underwear_bottom with states worn, removed, open, unknown. Copy the real Story substring and omit unobserved fields.',
  'Every retained narrow projection requires exact Story evidence. Open facts are the required path for observed emotion, relationship, agreement, refusal, physical, posture, clothing/accessory, intimate, sexual, or other meaning that does not fit a compact projection. Subject/object IDs must be registered and story_quote must be an exact contiguous substring. Do not emit semantic event or relation arrays for fresh Extract; counters and projections never decide whether an open fact occurred.',
  ...
  'Fresh block_observations must contain one matching block_id/block_type entry per story_observation_blocks item. Facts are nested; facts: [] means zero facts. Each nested fact contains only registered subject/object ids, fact_text, and an exact story_quote. Do not omit or invent blocks.',
  ...
  return [
    { role: 'system', content: SYSTEM_INSTRUCTIONS },
    {
      ...
      content: JSON.stringify({
        ...
        story_observation_blocks: buildStoryObservationBlocks(parsedStory).map(({ block_id, block_index, block_type, text }) => ({ block_id, block_index, block_type, text })),
