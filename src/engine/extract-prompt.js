...
const SYSTEM_INSTRUCTIONS = [
  ...
  'Explicit player physical continuity: preserve the observable kind and strength of player contact or sexual facts in Story evidence; do not euphemize them into an unidentifiable thing or pressure.',
    ...
    'CSA is not a fresh Extract semantic channel. Observe only narrow physical/clothing facts and the retained direct player sexual mechanic when exact Story evidence supports it; do not emit csa attitudes, runtime, aftereffects, generic stats, events, target or media fields. For the retained player sexual consumer, use the same actor evidence shape only: evidence.actors.player.changed may contain player_sexual_state.arousal_delta, player_sexual_state.ejaculation_progress_delta, player_sexual_state.erection_state, or player_sexual_state.ejaculation_completed. Never infer these from input, intent, clothing, CSA, images, or provider labels.'
  ]).join(' ');
