...
const SYSTEM_INSTRUCTIONS = [
  ...
  'Physical patches may set concise Korean location_label, posture, position_label from exact Story evidence; posture/position evidence quotes must be verbatim in Story and name the character. Clothing patches exist for both NPCs and the player and MUST use exactly the canonical four slots with enum values: state_delta.npc_scene_state[npc_id].clothing or state_delta.player_scene_state.clothing = { "uniform_top": "worn|removed|open|unknown", "uniform_bottom": "worn|removed|open|unknown", "underwear_top": "worn|removed|unknown", "underwear_bottom": "worn|removed|unknown" }. Free-form keys (bra, top, panties, 셔츠, 속옷, undergarments) are REJECTED — never output them. Clothing has one evidence channel only: top-level evidence.clothing = { "<actor_id>": { "<slot>": { "quote": "verbatim Story quote", "character_id": "<actor_id>" } } }; use actor_id "player" for the player. Do not put clothing evidence inside state_delta or a scene-state patch. Every proposed slot needs its own evidence quote. For an NPC the quote must name that NPC; for the player it must unambiguously describe the player. A directly revealed previously-unknown slot is a valid proposal even if that garment was not changed this turn. Keep uncertain slots omitted. CSA rule text or required attire alone never creates actual clothing, and magical/planned-only changes are invalid.',
