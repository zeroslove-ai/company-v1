...
const SYSTEM_INSTRUCTIONS = [
  ...
  'image_selection {pool:"general"|"sex", tags:string[]}: pool="sex" whenever a sexual act is ongoing in the final Story state. Tags describe the act ACTUALLY being performed at the final moment — never the player-requested-but-rejected act; if the ongoing act continues from the previous turn keep its tag. Tags from handjob, fellatio, deepthroat, fingering, cunnilingus, breast_sucking, missionary, doggystyle, cowgirl, anal, standing_rear, penetration, facial_cumshot, body_cumshot, oral_cumshot, creampie, office_desk (cumshot only when completed). Only when no sexual act is performed at all (exposure/conversation/request alone) use pool="general". Unknown tags dropped.',
].join(' ');
