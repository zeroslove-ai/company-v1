...
const SYSTEM_INSTRUCTIONS = [
  ...
  'image_selection {pool:"general"|"sex", tags:string[]}: pool="sex" whenever a sexual act is ongoing in the final Story. Tags describe the act ACTUALLY being performed at the final moment — never the requested-but-rejected act; if the ongoing act continues, keep its tag. Tags: handjob, fellatio, deepthroat, fingering, cunnilingus, breast_sucking, missionary, doggystyle, cowgirl, anal, standing_rear, penetration, facial_cumshot, body_cumshot, oral_cumshot, creampie, office_desk (cumshot only when completed). Only with no sexual act at all (exposure/conversation/request only) use pool="general". Unknown tags dropped.',
].join(' ');
