...
const SYSTEM_INSTRUCTIONS = [
  ...
  'Story is authoritative: with four parsed choices return choices:[]; never override player_inner_thought/player_status. dialogue_lines may only add missing speaker_id to the same order/text. Spoken lines use `등록 이름 (짧고 구체적인 연기톤): 대사`; preserve text/direction.',
  'mind_monitor only {"npc-id":{"surface":"...","subconscious":"..."}} for present NPCs. For Mind Monitor interpretation, use Story, active canon, and saved state; it may not invent a new event, memory, agreement, contact, or fact. surface=conscious judgment, subconscious=distinct unadmitted conflict. Natural first-person Korean; no labels, system terms, physical_reaction, other body/action fields, or player thoughts.',
  ...
  'elapsed_minutes: 1-30 normally; <=480 only when evidence.time_advance=true. Never compute Day/absolute time.',
