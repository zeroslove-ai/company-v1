...
const SYSTEM_INSTRUCTIONS = [
  ...
  'Explicit player physical continuity: preserve the observable kind and strength of player contact or sexual facts in Story evidence; do not euphemize them into an unidentifiable thing or pressure.',
    ...
    'Exact evidence contract: evidence is a top-level sibling of player_observation and npc_observations. Never put an evidence key inside a player or NPC object. Use evidence.clothing.<actor_id>={quote,character_id} and one actor-scoped evidence.physical_change.<actor_id>={character_id,changed:[path],quote}; changed paths must be exact canonical paths such as npc_scene_state.heroine2.position_label, npc_scene_state.heroine2.clothing.underwear_bottom, or player_scene_state.position_label. Scene evidence uses {kind,character_id or location_id,quote}; kind is presence, entrance, exit, or scene. Entrance/exit require registered ids and exact contiguous Story quotes. Copy quotes verbatim from story_text; never compose a quote from inferred facts or any input outside story_text. Omit unsupported or unobserved evidence; locations must be registered. For NPC physical evidence, character_id and the actor-scoped key must identify the same registered actor.',
    'Illustrative physical shape (not mandatory output): {"npc_observations":{"heroine2":{"physical":{"position_label":"exact observed label","clothing":{"underwear_bottom":"removed"}}}},"evidence":{"clothing":{"heroine2":{"character_id":"heroine2","quote":"exact Story substring"}},"physical_change":{"heroine2":{"character_id":"heroine2","changed":["npc_scene_state.heroine2.position_label","npc_scene_state.heroine2.clothing.underwear_bottom"],"quote":"same exact substring"}}}}. Use position_label only; do not emit a closed position enum or generic label field. In a multi-NPC scene, include the actor name in physical/clothing quotes. Clothing slots are uniform_top, uniform_bottom, underwear_top, underwear_bottom with states worn, removed, open, unknown. Copy the real Story substring and omit unobserved fields.',
    'CSA is not a fresh Extract semantic channel. Observe only narrow physical/clothing facts and the retained direct player sexual mechanic when exact Story evidence supports it; do not emit csa attitudes, runtime, aftereffects, generic stats, events, target or media fields. For the retained player sexual consumer, use exact Story evidence only: evidence.player_observation.sexual.arousal_delta={quote}, evidence.player_observation.sexual.ejaculation_progress_delta={quote}, evidence.player_erection={state,quote}, and evidence.sexual_resolution=true only when the Story explicitly shows completion. Never infer these from input, intent, clothing, CSA, images, or provider labels.'
  ]).join(' ');
