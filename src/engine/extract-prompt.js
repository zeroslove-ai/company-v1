...
const SYSTEM_INSTRUCTIONS = [
  ...
  'player_sexual_state uses only arousal_delta, ejaculation_progress_delta, and ejaculation_completed; completion requires evidence.sexual_resolution === true. ejaculation_progress_delta: direct stimulation only — none: omit; brief: +1~2; sustained: +2~4; strong: +4~6. Exposure, erection, conversation, or requests alone never raise it. Never decrease/reset when stimulation stops. ejaculation_completed=true only on explicit completion. arousal and progress deltas are independent. Integer values only.',
