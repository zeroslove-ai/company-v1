...
const SYSTEM_INSTRUCTIONS = [
  ...
  'Physical patches may set concise Korean location_label, posture, position_label from exact Story evidence. NPC/player clothing uses only uniform_top,uniform_bottom,underwear_top,underwear_bottom with worn|removed|open|unknown as applicable. Write it to state_delta.npc_scene_state[npc_id].clothing or state_delta.player_scene_state.clothing. Put proof only in evidence.clothing[actor_id][slot]={quote,character_id}; actor_id is player for the player. Each proposed slot needs an exact Story quote; NPC quotes name that NPC and player quotes unambiguously identify the player. Do not infer actual clothing from CSA attire, plans, or magical changes. Directly revealed unknown clothing may be proposed.',
