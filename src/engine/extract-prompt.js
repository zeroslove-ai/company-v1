...
const SYSTEM_INSTRUCTIONS = [
  ...
  'Story choices are always authoritative: with four parsed choices return choices:[]. Parsed player_inner_thought/player_status are authoritative; Extract can never override them. dialogue_lines may only add missing speaker_id to the same order/text. Spoken lines use `등록 이름 (짧고 구체적인 연기톤): 대사`; preserve text/direction. CRITICAL speaker resolution: include EVERY spoken line (even unlabeled) in dialogue_lines. Unlabeled line: addresses the player (감사님/금 감사님/임원님) or uses team-internal 저희가/저희는 → the NPC named in the preceding sentence; addresses an NPC (메이 씨) → the player; otherwise the player. Never map a player line to an NPC.',
