...
function sceneContext(save, id) {
  ...
  return Object.fromEntries(['present', 'location_id', 'position_label', 'clothing']
...
const SYSTEM_INSTRUCTIONS = [
  ...
  `Output exactly this V2 JSON shape (values may contain only observed data): ${JSON.stringify({
   ...
   'Return only the JSON schema below. Do not include arbitrary save patches or parser projections.',
  ...
  'Parser projections remain the source for displayed selections, spoken-line order, and player inner monologue. Mind Monitor is a separate presentation projection grounded in the supplied Story and target character.',
  'Return narrow player_observation and npc_observations projections only when a proven product/UI or mechanical consumer needs them. Compact clothing may use the four existing UI slots; one current physical erection_state may be retained when directly evidenced, but do not emit numeric sexual counters or event history. Never propose scene, location, presence, or ID fields through a projection.',
   'Return npc_observations only for registered NPCs and only for narrow projections that the current product consumes.',
  'The top-level keys of npc_observations must be registered NPC IDs, never a domain key; for example: {"npc_observations":{"heroine2":{"physical":{"position_label":"창가 쪽에 서 있다"}}}}.',
    'If a narrow projection has no exact observed change, omit it. Ordinary dialogue may produce empty projections and scene_observation with final_present_npc_ids:null.',
    'Every retained narrow projection requires exact Story evidence. Other narrative meaning remains in the raw Story and turn_summary.',
  ...
  'Explicit player physical continuity: preserve the observable kind and strength of player contact or sexual facts in Story evidence; do not euphemize them into an unidentifiable thing or pressure.',
    ...
    'Exact evidence contract: evidence is a top-level sibling of player_observation and npc_observations. Never put an evidence key inside a player or NPC object. Use one actor-scoped object only: evidence.actors.<actor_id>={character_id,quote,changed:[path]}. Each changed path must be exact, such as npc_scene_state.heroine2.position_label, npc_scene_state.heroine2.clothing.underwear_bottom, player_scene_state.position_label, or player_sexual_state.erection_state. Scene evidence remains scene_observation.evidence with {kind,character_id or location_id,quote}; kind is presence, entrance, exit, or scene. Copy quotes verbatim from story_text; never compose a quote from inferred facts or any input outside story_text. Omit unsupported or unobserved evidence; locations and actor ids must be registered. The actor key and character_id must identify the same actor.',
    'Illustrative physical shape (not mandatory output): {"npc_observations":{"heroine2":{"physical":{"position_label":"exact observed label","clothing":{"underwear_bottom":"removed"}}}},"evidence":{"actors":{"heroine2":{"character_id":"heroine2","changed":["npc_scene_state.heroine2.position_label","npc_scene_state.heroine2.clothing.underwear_bottom"],"quote":"same exact Story substring"}}}}. Use position_label only; do not emit a closed position enum or generic label field. In a multi-NPC scene, include the actor name in physical/clothing quotes. Clothing slots are uniform_top, uniform_bottom, underwear_top, underwear_bottom with states worn, removed, open, unknown. Copy the real Story substring and omit unobserved fields.',
    'Use the same actor evidence shape for retained physical, clothing, and current physical erection_state projections. Exact Story evidence is required for each changed path; evidence.actors.player.changed may contain only player_sexual_state.erection_state for this retained physical fact. Do not infer a changed path from player input alone.'
  ]).join(' ');
