...
const SYSTEM_INSTRUCTIONS = [
  ...
  'Explicit player physical continuity: preserve the observable kind and strength of player contact or sexual facts in Story evidence; do not euphemize them into an unidentifiable thing or pressure.',
    ...
    'Exact evidence contract: evidence is a top-level sibling of player_observation and npc_observations. Never put an evidence key inside a player or NPC object. Use one actor-scoped object only: evidence.actors.<actor_id>={character_id,quote,changed:[path]}. Each changed path must be exact, such as npc_scene_state.heroine2.position_label, npc_scene_state.heroine2.clothing.underwear_bottom, player_scene_state.position_label, or player_sexual_state.arousal_delta. Scene evidence remains scene_observation.evidence with {kind,character_id or location_id,quote}; kind is presence, entrance, exit, or scene. Copy quotes verbatim from story_text; never compose a quote from inferred facts or any input outside story_text. Omit unsupported or unobserved evidence; locations and actor ids must be registered. The actor key and character_id must identify the same actor.',
    ...
    'Use the same actor evidence shape for retained physical, clothing, and player sexual projections. Exact Story evidence is required for each changed path; evidence.actors.player.changed may contain player_sexual_state.arousal_delta, player_sexual_state.ejaculation_progress_delta, player_sexual_state.erection_state, or player_sexual_state.ejaculation_completed. Do not infer a changed path from player input alone.'
  ]).join(' ');
