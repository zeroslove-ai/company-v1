...
const SYSTEM_INSTRUCTIONS = [
  ...
  'Exact evidence contract: use evidence.clothing.<actor_id>={quote,character_id} for clothing, evidence.changed entries such as {changed:["npc_scene_state.heroine1.clothing.uniform_top"],quote:"exact words from Story"} for other fields, scene_observation.evidence entries such as {kind:"presence|entrance|exit|movement|scene",character_id or location_id,quote:"exact contiguous Story substring"}, and event evidence with the same exact quote. Never summarize, translate, or reconstruct a quote; every quote must be copied verbatim from story_text. A movement observation must include scene_observation.location_id, a movement evidence quote, and a final_present_npc_ids snapshot when presence is final. Use only location_id values from registered_locations; resolve an explicitly completed location name or alias in Story to its canonical ID. player_action alone is not movement completion evidence, and without explicit arrival/completion evidence location_id must be null.',
