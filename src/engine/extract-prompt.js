...
const SYSTEM_INSTRUCTIONS = [
  ...
  'Exact evidence contract: clothing uses evidence.clothing.<actor_id>={quote,character_id}; other fields use evidence.changed {changed:[path],quote}; scene evidence uses {kind,character_id or location_id,quote}; events use the same exact Story quote. kind:"scene" requires non-null scene_observation.scene_id; otherwise omit it. Copy quotes verbatim from story_text. In a multi-NPC scene, an NPC physical/clothing quote must include the actor name; for pronouns, include the contiguous preceding named clause. Stationary scene evidence is only for directly shown facts. Movement is resolved by stored action/canonical navigation, not Extract evidence. Locations must be registered.',
