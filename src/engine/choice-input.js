 ...
 * previously-rendered choice text, so a plain "2" click/keystroke submits the SAME text
 * as clicking the rendered choice would — never a separate, looser "the player typed a
 * number" interpretation. If the input isn't one of these forms at all, this resolves to
 * null and the caller treats it as ordinary free-typed text. 선택지 metadata는 사용하지 않는다.
 */
 ...
 * - { ok: true, choice_index, text } on a valid resolution
 */
