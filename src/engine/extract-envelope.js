...
const KNOWN_FIELDS = new Set(['state_delta', 'outcome', 'evidence', 'turn_summary', 'mind_monitor', 'choices', 'dialogue_lines']);
...
export function normalizeExtractEnvelope(value) {
  ...
  const warnings = Object.keys(value).filter(key => !KNOWN_FIELDS.has(key)).map(key => `unknown_extract_field:${key}`);
