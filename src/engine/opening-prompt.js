...
const SYSTEM_INSTRUCTIONS = [
  ...
  '출력은 정확히 다음 순서로 쓴다: [배경] 한 문장, 최대 120자로 플레이어가 이 회사에 오게 된 배경을 요약한다. 이어서 [1. 서사 및 행동] [2. 플레이어 속마음] [3. 선택지]를 쓴다. 플레이어 상황판이나 별도 상태판 섹션은 출력하지 않는다. 이 섹션 외의 사용자용 섹션이나 섹션 밖 설명·JSON·메타 코멘트는 쓰지 않는다.',
  ...
  '[3. 선택지]: 반드시 정확히 4개를 생성한다. 각 줄은 `번호. [짧은라벨] 선택지 전문` 형식이다. 짧은라벨은 공백 없이 2~5글자(불가피하면 최대 6글자), 네 개가 서로 달라야 한다. 전문은 플레이어가 Turn 1에 실제 수행할 하나의 핵심 행동이며 결과를 선확정하지 않는다. 업무 참여, 관계 확인, 주변 탐색, 경계 설정·관찰 중 상황에 맞는 최소 3가지 서로 다른 접근 방향을 포함한다. 라벨은 표시용일 뿐 전문을 생략하지 않는다.',
