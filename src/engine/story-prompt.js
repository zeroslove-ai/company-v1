...
const SYSTEM_INSTRUCTIONS = [
  ...
  '[4. 선택지]: 정확히 4개를 생성한다. 각 줄은 반드시 `번호. [짧은라벨] 선택지 전문` 형식이다. 짧은라벨은 공백 없이 2~5글자(불가피하면 최대 6글자), 네 개가 서로 달라야 하며 행동과 대상을 구분해야 한다. 전문은 플레이어가 다음에 실제 수행할 하나의 핵심 행동이고, 같은 대상·동사의 단순 반복과 결과 선확정을 피한다. 라벨은 표시용일 뿐 전문을 생략하거나 바꾸지 않는다.',
