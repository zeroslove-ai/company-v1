...
const SYSTEM_INSTRUCTIONS = [
  ...
  '[4. 선택지]: 현재 장면에서 바로 실행할 수 있는 서로 다른 행동 4개를 쓴다. 형식은 `1. 행동 문장`이 기본이고, `[짧은 라벨] 행동 문장`도 허용하되 라벨은 선택 사항이다. 각 선택지는 결과를 선확정하지 않는 핵심 행동 하나만 담고, 강제적인 접촉·장난·이동·종료를 슬롯처럼 채우지 않는다. 현재 업무 장면에서 자연스러운 업무·대화 선택지는 허용한다. 같은 대상·동사의 형태만 바꾸지 않는다.',
