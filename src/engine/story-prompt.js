...
export const PROVIDER_CHOICE_OUTPUT_PROTOCOL = [
  ...
  'Each block must contain one non-empty concrete player-action proposal.',
  'The four choices must be distinct literal strings; preserve their exact text.',
  'Choices are proposals, not completed player actions.',
  'Do not emit server/UI numbering or a human heading or numbered list; the UI owns numbering.',
  'Before ending the response, verify that exactly four choice blocks are present and each is non-empty and distinct.'
].join(' ');
...
export const DURABLE_STORY_RULES = [
 ...
 '[CSA AND WORLD RULES]',
  ...
  'Every response must contain at least one non-empty player-visible Story body segment: plain narrative text, [SCENE], [DIALOGUE], or visible [ACTING] text. A [THOUGHT] plus [CHOICE] blocks alone is invalid and does not count as Story body. Output one short player-only [THOUGHT] paragraph closed by [/THOUGHT].',
  'Write plain narrative by default, preserving source order. Mark each spoken line with [DIALOGUE speaker_id="registered_id_or_player"] using an exact registered ID; never infer a speaker from a name, quote, or previous line. Player posture may be structurally annotated only when the narrative visibly establishes it, and no ACTING token is required for CSA. Before ending, verify exactly one [THOUGHT] and that every spoken line has a DIALOGUE marker. Preserve the kind, strength, and scope of explicit player actions without strengthening them. Do not turn app, marker, and presentation mechanics into world knowledge.',
