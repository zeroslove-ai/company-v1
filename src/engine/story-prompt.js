...
function buildActiveWorldRules(save, expectedTurn = null) {
  ...
  return activeIds.flatMap(csaId => {
    ...
    return [{
      ...
      mode: preset.mode ?? semantic.trigger ?? 'continuous',
      roles: object(preset.roles) ?? {},
      applies_to: preset.roles?.subject_group ?? preset.roles?.performer_group ?? semantic.actor_group ?? 'declared_scope',
      required_action: preset.required_action ?? rule.required_action ?? null,
      sexual_actions: Array.isArray(preset.sexual_actions) ? preset.sexual_actions : [],
      method_policy: preset.method_policy === 'restricted' ? 'restricted' : 'unspecified',
...
const SYSTEM_INSTRUCTIONS = [
  ...
  '[COMMON-SENSE CHANGE RULES] active_world_rules만 현재 상식의 단일 권위다. 각 rule의 roles·mode·required_action과 정확히 맞는 요청은 역할 범위의 현재 인물 모두에게 같은 행동 결과를 적용하고, 성격과 감정은 표정·말투·속도에만 반영한다. 조건·이행·과거 사건을 창작하지 않으며 unspecified method는 열린 결과 규칙이다. NPC는 앱·CSA·내부 ID를 말하지 않고, 직접 질문이 없으면 규정 설명보다 장면 진행을 우선한다. 일반 요청은 장면과 관계에 따라 자연스럽게 처리한다. 플레이어의 자유 입력 자체는 막지 않는다.',
