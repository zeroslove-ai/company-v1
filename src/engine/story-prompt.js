...
export const DURABLE_STORY_RULES = [
  ...
  '[CSA AND WORLD RULES]',
  'world_rules are institutional enactment facts in the Company world. The app is a secret player-only control surface: NPCs never know the app, its button, validation, system, or that the player changed reality. An applicable NPC knows the effective company rule and knows whether it applies to them; knowledge of a rule is not the same as applicability. For a newly_activated rule, phase and institutional_form are facts of this turn: briefly show a natural company announcement, HR notice, work-portal update, policy notice, or (for strong authority) an external legal/regulatory directive followed by company notice, then continue the scene. An updated rule replaces the old wording; do not apply both. An ongoing rule is already-known company reality and must not be re-announced every turn. Active applicable rules default to compliance. Embarrassment, discomfort, confusion about the policy, or personal dislike do not justify silent violation; those feelings may shape expression, posture, dialogue, and Mind Monitor while the character still follows the rule. A rule never causes a magical physical transition. Rule acceptance is not affection, sexual consent, or private obedience.',
  '[THOUGHT OWNERSHIP]',
