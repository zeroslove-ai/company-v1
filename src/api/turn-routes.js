        ...
        try {
          ...
          if (movementContract.transition_mode === 'movement' && movementContract.destination_location_id) {
            ...
            if (destination) {
              messages = [{
                ...messages[0],
                content: `${messages[0].content}\n\n[MOVEMENT OBSERVATION CONTRACT] This is an explicit movement turn. The requested canonical destination is ${destination.name} (location_id=${destination.location_id}). If the raw Story visibly arrives there, this movement contract requires a final presence snapshot: set scene_observation.location_id to exactly ${destination.location_id}, set presence_is_final=true, and return final_present_npc_ids as the exact registered NPC snapshot visibly remaining at the destination (use [] when no registered NPC is explicitly identified, even if unnamed employees are mentioned). Include an exact contiguous movement evidence quote from the Story. For NPCs present before the move but not remaining at the destination, include one exact exit evidence entry per registered character; for registered NPCs visibly remaining at the destination, include one exact presence evidence entry per character. Do not encode generic or unregistered people as presence evidence: a presence entry always requires a concrete registered character_id. For an empty destination use final_present_npc_ids=[] and no presence evidence entry. Do not substitute the origin location, and do not infer arrival from player_action alone. If the Story does not visibly arrive there, keep location_id null, presence_is_final=false, final_present_npc_ids=null, and provide no movement evidence.`
              }, ...messages.slice(1)];
            }
          }
