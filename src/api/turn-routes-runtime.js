...
function activeRulesSection(save) {
  ...
  const lines = active.length
    ? active.map(item => `- (${item.id}) [${item.strength || 'weak'}] ${item.content || ''}`).join('\n')
    : '- 없음';
  return `\n\n[POST-TRANSACTION ACTIVE CSA SET — FINAL AUTHORITY]\n아래 목록이 이번 Story/Extract 턴에 실제로 활성인 전체 상식개변이다. 앞선 Context나 application-check에 이 목록에 없는 ID가 남아 있다면 무시하고, 아래 목록의 ID·강도·내용만 적용한다.\n${lines}`;
}
