...
import { applyR3Csa, buildRuleChangeStoryBinding, createR3CsaCatalog } from '../domain/csa.js';
...
async function processTurn({ store, provider, content, gameId, job, emit }) {
  const startedAt = Date.now(); const mark = (stage, details) => timingMark(emit, startedAt, stage, details);
  const before = await store.context(gameId); const csaOperation = job.csa_operation ?? job.rule_change_event ?? null; const ruleChangeEvent = csaOperation ? applyR3Csa({ state: before.state.state, content, rawOperations: [csaOperation], catalog: createR3CsaCatalog(content.csaPresets) }).last_rule_change : null; const ruleChangeBinding = ruleChangeEvent ? buildRuleChangeStoryBinding({ event: ruleChangeEvent, content }) : null; const attempt = { gameId, turnNumber: job.turn_number, actionId: job.action_id, attemptNo: job.attempt_no, literalAction: job.literal_action, ...(csaOperation ? { csaOperation } : {}) }; const navigationIntent = resolvePlayerNavigationIntent({ content, state: before.state.state, literalAction: attempt.literalAction }); const storyContext = projectNavigationContext(before, navigationIntent, content); const storyLiteralAction = csaOperation ? '' : attempt.literalAction; let storyText = ''; let lastProgress = 0; let writes = 0;
