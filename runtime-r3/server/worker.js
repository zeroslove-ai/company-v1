...
import { applyR3Csa, buildRuleChangeInstitutionalAnnouncement, buildRuleChangeStoryBinding, createR3CsaCatalog } from '../domain/csa.js';
...
async function startTurn(store, provider, content, gameId, input, executionCtx = null) {
  ...
  return streamTurn({ store, provider, content, gameId, job: { ...reservation.job, csa_operation: reservation.job?.rule_change_event ?? csaOperation }, executionCtx });
}
...
async function processTurn({ store, provider, content, gameId, job, emit }) {
  const startedAt = Date.now(); const mark = (stage, details) => timingMark(emit, startedAt, stage, details);
  const before = await store.context(gameId); const csaOperation = job.csa_operation ?? job.rule_change_event ?? null; const ruleChangeEvent = csaOperation ? applyR3Csa({ state: before.state.state, content, rawOperations: [csaOperation], catalog: createR3CsaCatalog(content.csaPresets) }).last_rule_change : null; const ruleChangeBinding = ruleChangeEvent ? buildRuleChangeStoryBinding({ event: ruleChangeEvent, content }) : null; const institutionalAnnouncement = ruleChangeEvent ? buildRuleChangeInstitutionalAnnouncement({ event: ruleChangeEvent, binding: ruleChangeBinding }) : ''; const attempt = { gameId, turnNumber: job.turn_number, actionId: job.action_id, attemptNo: job.attempt_no, literalAction: job.literal_action, ...(csaOperation ? { csaOperation } : {}) }; const navigationIntent = resolvePlayerNavigationIntent({ content, state: before.state.state, literalAction: attempt.literalAction }); const storyContext = projectNavigationContext(before, navigationIntent, content); const storyLiteralAction = csaOperation ? '' : attempt.literalAction; let storyText = institutionalAnnouncement ? `${institutionalAnnouncement}\n\n` : ''; let lastProgress = 0; let writes = 0;
  emit('meta', { game_id: gameId, turn_number: job.turn_number, action_id: job.action_id }); mark('story_request_start');
  if (institutionalAnnouncement) emit('story_delta', { text: storyText });
  try {
    for await (const delta of provider.story({ literalAction: storyLiteralAction, context: storyContext, csaOperation, ruleChangeEvent, ruleChangeBinding, content, onTiming: mark })) { const text = String(delta); storyText += text; emit('story_delta', { text }); if (writes < R3_MAX_PROGRESS_WRITES && (writes === 0 || storyText.length - lastProgress >= R3_PROGRESS_INTERVAL_CHARS)) { await store.updateProgress({ gameId, turnNumber: attempt.turnNumber, attempt, storyText }); writes += 1; lastProgress = storyText.length; } }
    mark('story_complete'); await store.markStoryComplete({ gameId, turnNumber: attempt.turnNumber, attempt, storyText });
    let rawObserver = {}; let observerFailed = false; let observerFailureCode = null; let observerFailureWarnings = []; let observerFailureEvidence = {}; mark('observer_start'); try { rawObserver = await provider.observe({ literalAction: attempt.literalAction, storyText, context: storyContext, csaOperation, ruleChangeEvent, content }); mark('observer_complete'); } catch (error) { observerFailed = true; const failure = observerFailureState(error); observerFailureCode = failure.code; observerFailureWarnings = failure.warnings; observerFailureEvidence = failure.evidence; mark('observer_failed', { observer_error_code: observerFailureCode, ...observerFailureEvidence }); }
