...
import { buildFeedbackContext } from './store.js';
...
export class SupabaseR3Store {
  ...
  async presentationContext(gameId) {
    const [games, states] = await Promise.all([this.db.select('company_r3_games', { game_id: `eq.${gameId}` }), this.db.select('company_r3_state', { game_id: `eq.${gameId}` })]);
    if (!games?.[0] || !states?.[0]) throw new Error('r3_game_not_found');
  ...
  async feedbackContext(gameId, snapshot) { return buildFeedbackContext(await this.context(gameId), snapshot); }
  async beginFeedbackRevision({ gameId, revisionRequestId, expectedTurn, expectedStateRevision, feedbackText }) { const result = await this.db.rpc('company_r3_begin_feedback_revision', { p_game_id: gameId, p_revision_request_id: revisionRequestId, p_expected_turn: expectedTurn, p_expected_state_revision: expectedStateRevision, p_feedback_text: feedbackText }); return { created: result?.created === true, attempt: result?.attempt ?? null, snapshot: result?.snapshot ?? null }; }
  async commitFeedbackRevision({ gameId, attempt, revisionRequestId, expectedTurn, expectedStateRevision, storyText, choices, summary, mindMonitor, observerRaw, observerApplied, warnings, stateAfter }) { const result = await this.db.rpc('company_r3_commit_feedback_revision', { p_game_id: gameId, p_attempt_id: attempt.attemptId, p_revision_request_id: revisionRequestId, p_expected_turn: expectedTurn, p_expected_state_revision: expectedStateRevision, p_story_text: storyText, p_choices: choices ?? [], p_turn_summary: summary, p_mind_monitor: mindMonitor ?? {}, p_observer_raw: observerRaw ?? {}, p_observer_applied: observerApplied ?? {}, p_warnings: warnings ?? [], p_state_after: stateAfter }); return { ...await this.context(gameId), feedback: result }; }
  async failFeedbackRevision({ gameId, attemptId, revisionRequestId, errorCode }) { return this.db.rpc('company_r3_fail_feedback_revision', { p_game_id: gameId, p_attempt_id: attemptId, p_revision_request_id: revisionRequestId, p_error_code: errorCode }); }
  async applyCsa({ gameId, expectedRevision, stateAfter, operations = [] }) { await this.db.rpc('company_r3_apply_csa', { p_game_id: gameId, p_expected_revision: expectedRevision, p_state_after: stateAfter, p_operations: operations }); return this.context(gameId); }
