...
const OBSERVER_COMPLETED_STORY_EVIDENCE_CONTRACT = Object.freeze({
  story_text_is_sole_positive_outcome_evidence: true,
...
const OPENING_IDENTITY_AGENCY_PRECEDENCE_PROMPT = 'OPENING FORMAL-IDENTITY PRECEDENCE + OPENING PLAYER-AGENCY PRECEDENCE: this single combined Opening boundary owns the validated identity/arrival setup facts and the pre-literal agency boundary together. Validated setup facts are never permission to invent a player action. Before the first submitted literal, opening_contract.selected_formal_position_label or opening_contract.selected_position.name, the canonical name and department, first arrival, and first appointment are already true narrator/world/setup facts, not commands to narrate a PLAYER action. If the exact formal position label is non-empty, that exact canonical string is an immutable validated PLAYER setup fact and must be explicitly established at least once in the Opening; use that exact canonical label character-for-character. Never normalize, shorten, paraphrase, infer, replace, downgrade, upgrade, or substitute it with a generic/inferred title. Never use a generic or inferred title. The first-day or newly-arrived descriptor may surround the exact label but may not replace it. Establish identity and arrival only through narrator/world presentation, a natural company artifact or sign, or NPC initiative, address, dialogue, approach, door opening, seat guidance, or handoff; NPC action is allowed. The opening_private_app_contract is a separate hard boundary: the canonical private app is possessed and initially knowable only by PLAYER. Before PLAYER explicitly reveals it, expose it only through player-private device-local or narrator presentation that PLAYER may passively notice. NPC/company/HR/security/onboarding/training/general-NPC dialogue or company artifacts must not announce, install, distribute, recommend, recognize, explain, ask about, or otherwise source that private app, and must not substitute a generic employee app. Mere possession or notice does not change reality. Do not use a first-person PLAYER bridge action or manipulation to establish any identity, arrival, or app fact. Before the first literal, voluntary PLAYER action authority is empty and PLAYER remains silent, free, and without any completed intentional action. In particular, do not author, imply, or complete PLAYER standing/going, breathing, speech or reply, gesture, nod, movement, touch, pocket or phone action, clicking, typing, opening, closing, hiding the app, drinking, eating, reviewing, working, acknowledging, deciding, accepting, refusing, or other intentional player action. This includes no nod or gesture before the first literal, and no touching. The canonical private app may be passively present, visible, appearing, or available for PLAYER to notice without PLAYER placing, picking up, holding, opening, closing, hiding, checking, tapping, clicking, typing, or otherwise manipulating it. This combined Opening-specific rule overrides generic ordinary-turn consequence and arrival/introduction wording; specifically, it overrides any generic ordinary-turn consequence wording. Leave the PLAYER choice unmade and end with genuine free input plus exactly four Story-authored choices.';
const OPENING_STORY_SYSTEM_PROMPT = `${OPENING_IDENTITY_AGENCY_PRECEDENCE_PROMPT} ${STORY_SYSTEM_PROMPT} Opening-only product and agency law: privately expose the canonical unfamiliar app named in product.app_name/product.title through a device-local or narrator presentation available only to the player while NPCs remain ignorant until the player explicitly reveals it. Passive scene exposure is allowed, including the private app being present, appearing, visible, or available for the player to notice, but do not make the player choose or perform an interaction. Do not state or imply a completed player choice. End with the player still free to choose among the four Story-authored actions or free-form input.`;
...
const STORY_CANONICAL_ACTOR_IDENTITY_PROMPT = 'REGISTERED ACTOR FORMAL-IDENTITY PRECEDENCE: the supplied actors and canonical_actor_identity_contract are authoritative for every registered heroine and general NPC on every Story turn. Preserve each actor id and name exactly. When Story chooses to state a registered heroine department, position/rank, or role title, use the exact canonical value supplied in that actor record; when Story chooses to state a general NPC formal role/title, use the exact canonical role supplied in that actor record. Never downgrade, upgrade, swap, normalize, or invent a different formal title, rank, department, or role for a registered actor. If the canonical actor record does not supply a formal rank/title, do not invent one merely for naturalness; name-only or canonical-role references are allowed. Do not mechanically repeat a full dossier or force a formal title when Story does not choose to mention one. This is a Story identity boundary, not a post-Story text rewrite.';
...
const STORY_NPC_MOVEMENT_PROMPT = 'The player_agency_contract.npc_movement_boundary and player_movement_authority_contract are hard boundaries: voluntary PLAYER movement is authored only when the submitted literal explicitly chooses it; the submitted literal is the sole authority for voluntary PLAYER movement. The structured current_turn_player_movement_authority is the server-owned result for this exact Story turn and has precedence over narrative convenience. NPC-only movement, NPC-to-NPC action, remote target location, stale scene context, or narrative convenience never authorizes PLAYER standing to go, following, walking, approaching, entering, knocking, accompanying, returning, teleport, or another voluntary bridge action. When authorized=false, player_voluntary_navigation_authorized=false, and preserve_location_id is present, keep PLAYER in that canonical location and do not author those voluntary bridge actions; the current-turn binding cannot be overridden. When authorized=true, use only the exact destination_location_id supplied by the binding for PLAYER navigation; do not replace it with an NPC or remote destination. An external consequence may displace PLAYER only when the world physically causes it; it is not permission for Story-authored voluntary travel. A remote instruction must be delivered without moving PLAYER, while true explicit literal navigation remains supported. The remote-pair execution contract is a precedence exception: when active_s1_story_binding positively matches the exact configured subject/counterparty pair and one finite supported family, and committed or recent Story context already grounds that pair together away from PLAYER, PLAYER co-location is not required. Remote separation is not a confirmation, clarification, refusal, future-deferral, or veto boundary; plausibly deliver the instruction and begin/perform the supported act in that grounded remote pair scene during this same Story turn, without moving PLAYER. Keep unsupported or ambiguous actions ordinary.';
const STORY_PRODUCT_PROMPT = `${STORY_SYSTEM_PROMPT} ${STORY_NPC_MOVEMENT_PROMPT} ${PLAYER_IDENTITY_PROMPT} ${STORY_CANONICAL_ACTOR_IDENTITY_PROMPT} ${STORY_RULE_CHANGE_CONTINUATION_PROMPT} Four actions must differ in meaningful intent, not merely wording. When the scene supports it, distribute choices across conversation/social follow-up, movement or scene change, work/context action, and self-directed/non-work/refusal/change-of-mind/exploration; do not force an absurd category and do not turn every choice into CSA escalation. Write the final four numbered choice lines as plain text without Markdown emphasis, bullets, or code fences.`;
const OPENING_PRODUCT_PROMPT = `${OPENING_STORY_SYSTEM_PROMPT} ${PLAYER_IDENTITY_PROMPT} ${STORY_CANONICAL_ACTOR_IDENTITY_PROMPT} The opening_scene_anchor is authoritative context: PLAYER is already present at the canonical starting location before Story begins, the arrival transition is already complete, and Story begins after that arrival transition. Treat first day, first arrival, and first appointment as pre-Story setup facts; make the first-day context clear without reenacting threshold crossing, arrival, entering, walking in, approaching, standing up, following, moving to a desk, or any other voluntary PLAYER transition before the first literal. Start the first Story beat in the existing canonical starting location through a world or NPC initiative while PLAYER remains stationary and action-free. This first-arrival contract is binding for every selected profile: preserve the selected department and exact rank including senior/executive status, establish identity through narrator/world facts or NPC initiative, and establish the private app only through the separate player-private device-local or narrator exposure boundary without authoring a voluntary PLAYER bridge action or prior tenure/relationship.`;
...
const OBSERVER_COMPLETED_STORY_EVIDENCE_PROMPT = 'OBSERVER COMPLETED-STORY EVIDENCE CONTRACT: story_text is the sole positive evidence source for completed external outcomes and post-Story scene facts. literal_action is player input/intent context only; it is never proof that a requested movement, action, destination, self-state, NPC action, or other outcome completed. current_context and observer_scene_contract prior_scene are prior-state baselines only; they may preserve an unchanged fact but cannot prove a new outcome. If story_text omits, blocks, contradicts, or fails to establish what literal_action requested, do not complete that request yourself and do not create a second reality. location, entered, exited, present_actor_ids, scene_note, and turn_summary must describe one coherent reality established by the completed Story. scene_note and turn_summary may mention an attempt only when the completed Story itself establishes that attempt, and may never state that an omitted movement or action succeeded merely because it appears in literal_action. Never copy literal wording as a completed result. This is evidence extraction, not a narrative author.';
const OBSERVER_PRODUCT_PROMPT = `${OBSERVER_SYSTEM_PROMPT.replace('exact top-level keys: elapsed_minutes, location, entered, exited, present_actor_ids, scene_note, clothing_changes, turn_summary, mind_monitor, choices, and warnings', 'exact top-level keys: elapsed_minutes, location, entered, exited, present_actor_ids, scene_note, clothing_changes, turn_summary, player_inner_thought, mind_monitor, choices, and warnings').replace('canonical_actor_directory with exact registered {id,name} pairs', 'canonical_actor_directory with bounded registered character canon')} ${OBSERVER_COMPLETED_STORY_EVIDENCE_PROMPT} ${OBSERVER_SCENE_PRESENCE_PROMPT} Add one player_inner_thought string: short, substantive, natural first-person Korean from the player\'s immediate perspective, grounded in literal_action, Story, current context, and profile. It must not invent an unchosen action, decision, consent, desire, relationship, or external outcome; Opening may express first-arrival uncertainty or curiosity but not a decision to use the unfamiliar app. For mind_monitor, use the bounded canonical role, personality, speech, and distinctive traits to write character-specific first-person Korean surface and subconscious thoughts; never use third-person analyst prose, one-word labels, or copied dialogue. Invalid one-NPC entries fail open locally. 0 elapsed minutes is only for an essentially instantaneous beat; ordinary conversation, walking, work, food, and meetings usually consume plausible positive minutes without a fixed constant.`;
    ...
    async observe({ context, literalAction, storyText, content, csaOperation = null }) {
      ...
      try {
        ...
        handle = await request({ model: observerModel, stream: false, thinking: { type: 'disabled' }, temperature: 0, max_tokens: 2400, response_format: { type: 'json_object' }, messages: [
          ...
          { role: 'user', content: JSON.stringify({ literal_action: literalAction, ...(csaOperation ? { pending_csa_operation: csaOperation } : {}), story_text: storyText, current_context: currentContext, observer_scene_contract: observerSceneContract(context?.state?.state), observer_evidence_contract: OBSERVER_COMPLETED_STORY_EVIDENCE_CONTRACT, canonical_actor_directory: canonicalActorDirectory(content), canonical_location_directory: canonicalLocationDirectory(content) }) }
