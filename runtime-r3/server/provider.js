...
export function createR3Provider({ env, fetchImpl = fetch, timeouts: overrides = {} } = {}) {
  ...
  return {
    async *story({ context, literalAction = '', opening = false }) { const response = await request({ model: storyModel, stream: true, thinking: { type: 'disabled' }, max_tokens: 5000, messages: [{ role: 'system', content: 'Write natural Korean Company interactive fiction. You are the only narrative author. Preserve the literal player action. Write a vivid player-visible scene and end with exactly four natural full-action next suggestions. Do not emit OOC, control markers, semantic taxonomies, outcome classifiers, or a second author voice.' }, { role: 'user', content: JSON.stringify({ opening, ...buildStoryContext(context, literalAction) }) }] }, timeouts.storyTotalMs, 'r3_story_timeout'); yield* readOpenAiStream(response, timeouts.storyFirstContentMs); },
