  ...
  return {
    ...
    async observe({ context, literalAction, storyText, content }) {
      const handle = await request({ model: observerModel, stream: false, thinking: { type: 'disabled' }, temperature: 0, max_tokens: 1600, response_format: { type: 'json_object' }, messages: [
        ...
        { role: 'user', content: JSON.stringify({ literal_action: literalAction, story_text: storyText, current_context: context?.state?.state, canonical_actor_directory: canonicalActorDirectory(content), canonical_location_directory: canonicalLocationDirectory(content) }) }
