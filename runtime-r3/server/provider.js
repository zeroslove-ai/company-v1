...
const STORY_PRODUCT_PROMPT = `${STORY_SYSTEM_PROMPT} Four actions must differ in meaningful intent, not merely wording. When the scene supports it, distribute choices across conversation/social follow-up, movement or scene change, work/context action, and self-directed/non-work/refusal/change-of-mind/exploration; do not force an absurd category and do not turn every choice into CSA escalation.`;
const OPENING_PRODUCT_PROMPT = `${OPENING_STORY_SYSTEM_PROMPT} This first-arrival contract is binding for every selected profile: explicitly establish first day/first arrival/first appointment, preserve the selected department and rank including senior/executive status, and give a plausible immediate arrival or introduction context without prior tenure or relationships.`;
...
const OBSERVER_ACCEPTANCE_PROMPT = `${OBSERVER_PRODUCT_PROMPT} player_inner_thought is required and must be non-empty whenever the completed Story gives any grounded player perspective; if the scene provides no special emotion, use a neutral first-person observation rather than omitting the key. Do not let an omitted or invalid player projection block Story commit.`;
