...
const STORY_NPC_MOVEMENT_PROMPT = 'The player_agency_contract.npc_movement_boundary is a hard boundary: NPC-only movement, NPC-to-NPC action, remote target location, stale scene context, or narrative convenience never authorizes PLAYER follow, entry, accompaniment, teleport, or other movement. Preserve the canonical player scene unless the literal explicitly binds player movement or an independently grounded external consequence does so.';
...
const OBSERVER_SCENE_PRESENCE_PROMPT = 'Scene fields are one post-Story snapshot: location is the player\'s canonical location, and present_actor_ids is the exact set of registered actors physically co-located in that player scene at Story end. Grounded entered/exited evidence must agree with present_actor_ids: a grounded exited actor is absent and a grounded entered actor is present. Do not preserve a copied previous actor list when grounded transition evidence contradicts it. scene_note must describe that same current player scene; it may mention a remote actor or event as history, but must not claim an actor left while present_actor_ids keeps that actor physically present. NPC-only movement never moves the player.';
