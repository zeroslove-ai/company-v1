...
const OBSERVER_ACCEPTANCE_PROMPT = `${OBSERVER_PRODUCT_PROMPT} ${OBSERVER_PRESENTATION_PROMPT} player_inner_thought is required and must be non-empty whenever the completed Story gives any grounded player perspective; if the scene provides no special emotion, use a neutral first-person observation rather than omitting the key. Do not let an omitted or invalid player projection block Story commit.`;
