...
export function createR3Provider({ env, fetchImpl = fetch, timeouts: overrides = {} } = {}) {
  ...
  return {
    async *story({ context, literalAction = '', opening = false, content, feedbackText = '', feedbackReferenceStory = '', onTiming = null }) {
      const handle = await request({ model: storyModel, stream: true, thinking: { type: 'disabled' }, max_tokens: 5000, messages: [
        { role: 'system', content: STORY_SYSTEM_PROMPT },
        { role: 'user', content: JSON.stringify({ opening, ...(feedbackReferenceStory ? { feedback_reference_story: String(feedbackReferenceStory).slice(0, 4000) } : {}), ...buildStoryContext(context, literalAction, { content, opening, feedbackText }) }) }
    ...
    async observe({ context, literalAction, storyText, content }) {
      const handle = await request({ model: observerModel, stream: false, thinking: { type: 'disabled' }, temperature: 0, max_tokens: 1600, response_format: { type: 'json_object' }, messages: [
        ...
        { role: 'user', content: JSON.stringify({ literal_action: literalAction, story_text: storyText, current_context: observerCurrentState(context?.state?.state), canonical_actor_directory: canonicalActorDirectory(content), canonical_location_directory: canonicalLocationDirectory(content) }) }
