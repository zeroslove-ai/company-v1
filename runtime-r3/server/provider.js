...
const OPENING_IDENTITY_PRECEDENCE_PROMPT = 'OPENING FORMAL-IDENTITY PRECEDENCE: when opening_contract.selected_formal_position_label or opening_contract.selected_position.name is non-empty, that exact canonical string is an immutable validated PLAYER setup fact and must be explicitly established at least once in the Opening. Whenever the Opening introduces, names, or describes the PLAYER formal position, rank, title, badge, or business identity, use that exact canonical label character-for-character. Never normalize, shorten, paraphrase, infer, replace, downgrade, upgrade, or supplement it with a generic or inferred title; a first-day or newly-arrived descriptor may surround the exact label but may not replace it. The selected name and department are exact and immutable too. This Opening-specific identity rule overrides generic introduction wording and remains separate from voluntary player agency.';
