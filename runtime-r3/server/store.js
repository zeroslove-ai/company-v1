...
export class InMemoryR3Store {
  ...
  reserveTurn({ gameId, turnNumber, actionId, literalAction, csaOperation = null, retryFailed = false }) {
    ...
    if (existing) { if (existing.status === 'failed' && retryFailed) { const now = this.now(); Object.assign(existing, { action_id: actionId, literal_action: literalAction, csa_operation: clone(csaOperation), status: 'processing', stage: 'reserved', stage_started_at: now, story_text: '', error_code: null, attempt_no: existing.attempt_no + 1, updated_at: now }); return { job: clone(existing), created: true, retried: true }; } return { job: clone(existing), created: false }; }
    const now = this.now(); const job = { game_id: gameId, turn_number: turnNumber, action_id: actionId, literal_action: literalAction, ...(csaOperation ? { csa_operation: clone(csaOperation) } : {}), status: 'processing', stage: 'reserved', stage_started_at: now, story_text: '', error_code: null, attempt_no: 1, progress_writes: 0, created_at: now, updated_at: now };
