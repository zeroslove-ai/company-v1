...
/* test('Mind Monitor renders a compact body plus preserved surface/subconscious lines', () => {
  ...
  withFakeDocument(() => {
    ...
    assert.equal(content.children[0].className, 'mind-monitor-body');
    assert.equal(content.children[0].children[0].className, 'mind-line');
    assert.equal(content.children[0].children[1].className, 'mind-line');
...
*/
test('Mind Monitor omits retired numeric reaction meters', () => {
  const model = buildCompanyGameViewModel(contextWithMindMonitor());
  withFakeDocument(() => {
    const container = new FakeNode('div', 'mind-monitor');
    renderMindMonitor(container, model.media.mind_monitor_entries, { preferredId: 'heroine3' });
    const content = container.children[1];
    assert.equal(content.children[0].className, 'mind-monitor-body');
    assert.equal(JSON.stringify(container).includes('mind-stat'), false);
  });
});
