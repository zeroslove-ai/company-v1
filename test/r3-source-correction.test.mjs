...
test('R3 choice tail preserves the exact Turn 7 choices when Story separates terminal lines with blank lines', () => {
  const choices = [
...
test('R3 player inner thought is a bounded committed JSON projection with no migration-shaped state', () => {
  ...
  assert.equal(normalized.player_inner_thought, '');
  ...
  assert.equal(context.turns[0].observer_applied.player_inner_thought, '');
