...
test('검토1b: 활성 CSA 턴은 사전 coverage 없이 선언적 world rule만 Story에 전달한다', async () => {
  ...
  const messages = JSON.parse(llmStory.body).messages;
  const prompt = messages.map(message => message.content).join('\n');
  const storyPayload = JSON.parse(messages.find(message => message.role === 'user').content);
  assert.equal(storyPayload.context.active_world_rules.length, 2, '활성 규칙은 단일 projection 목록에만 존재');
  const csa5 = storyPayload.context.active_world_rules.find(rule => rule.csa_id === 'csa_5');
  assert.ok(csa5, 'csa_5 명시');
  assert.ok(!('global_csa' in storyPayload.context), 'Story context에는 global_csa가 없다');
  assert.ok(!prompt.includes('[ACTIVE WORLD RULES'), 'legacy world-rule heading is absent from Story prompt');
  assert.ok(!prompt.includes('actor_id='), '선택된 actor id 없음');
  assert.ok(!prompt.includes('target_id='), '선택된 target id 없음');
  assert.ok(!prompt.includes('undefined'), 'undefined 없음');
  assert.equal(prompt.split(csa5.content).length - 1, 1, '규칙 원문은 한 번만 전달');
});
