-- REVIEW ARTIFACT ONLY — do not run during test-additive-schema-bridge-audit-v1.
--
-- This is the smallest additive TEST bridge proven by the audit. It is
-- intentionally outside supabase/migrations/ and does not touch the
-- migration ledger. Apply only later, as one explicit operator-approved
-- TEST-only SQL-file execution after the plan's preflight checks pass.
--
-- The live function currently names its second input p_require_scene while
-- the landed source names it p_allow_legacy. PostgreSQL does not permit
-- CREATE OR REPLACE FUNCTION to rename an input parameter. The bridge keeps
-- the existing name because all current callers are positional and the
-- boolean is intentionally ignored by the narrow structural contract. No
-- DROP is required.
...
-- The existing p_require_scene name is retained; this boolean is ignored by
-- the current narrow structural contract.
