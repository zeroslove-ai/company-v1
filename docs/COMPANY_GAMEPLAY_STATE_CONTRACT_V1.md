...
The five-character master contract reserves these fields for every character: `character_id`, `name`, `age`, `department`, `position`, `addressing_rules`, `appearance`, `personality`, `initial_relationship`, `initial_stats`, `initial_csa_attitudes`, `voice_id`, `storage_bucket`, `storage_prefix`, `primary_image_path`, and `adult_image_prefix`, plus the narrative-only fields documented in `docs/COMPANY_HEROINES_V1.md` (`role_title`, `company_tenure`, `public_role_summary`, `speech_style`, `habits`, `work_profile`, `relationship_hooks`, `csa_response_profile`, and, for the two youngest characters, `youngest_line`). A character ID is a permanent stable key; name and position changes never replace it. Storage prefixes are keyed by that ID, one primary image belongs to the character, and adult images have their own prefix. All five character IDs are resolved with `mapping_status: "resolved"`:
