...
Each global rule has one lifecycle: `active`, `applicable`, `temporarily_interrupted`, `suspended`, `completed`, or `deactivated`. `temporarily_interrupted` preserves the rule and its history; it is not deletion. An NPC may refuse an applicable rule, but a rule never blocks free player input. Story/Extract may propose evidence and a delta; guarded Commit alone validates lifecycle transitions.
...
`arousal` and `ejaculation_progress` are clamped to `0..100`; `ejaculation_count` is a non-negative integer. Extract supplies deltas only. `ejaculation_completed: true` requires explicit Story evidence (`sexual_resolution: true`) at Commit; otherwise it is a blocking error. Completion increments the count and resets both progress and arousal to 0. An absent sexual delta leaves state unchanged.
...
Only `surface` and `subconscious` display. `body`, `physical`, `body_reaction`, `physical_action`, and Korean body-reaction keys are removed with warnings. The normalizer never mutates its input, creates no absent NPC entry, and does not assume focal character equals last speaker. A legacy plain string is retained separately as `legacy_text` with a warning so it is not lost, but is not fabricated into a canonical NPC entry.
...
The five-character master contract reserves these fields for every eventual character: `character_id`, `name`, `age`, `department`, `position`, `addressing_rules`, `appearance`, `personality`, `initial_relationship`, `initial_stats`, `initial_csa_attitudes`, `voice_id`, `storage_prefix`, `primary_image_path`, and `adult_image_prefix`. A character ID is a permanent stable key; name and position changes never replace it. Storage prefixes are keyed by that ID, one primary image belongs to the character, and adult images have their own prefix. This PR does not look up Storage, assign a real ID, reuse Hospital `heroine1`–`heroine10` IDs, or infer any of the five real mappings.
