...
async function api(path, options = {}) {
  ...
  if (!payload.ok) throw new Error(payload.error?.message ?? payload.error?.code ?? '요청에 실패했습니다.');
  return payload.data;
}
...
function setStatus(message, error = false) {
  ...
  if (apiStatus) { apiStatus.textContent = error ? '●' : '●'; apiStatus.dataset.state = error ? 'error' : 'ready'; }
}
...
function renderHistory(turns, job) {
  ...
  if (job?.status === 'processing' && job.story_text) {
    const progress = document.createElement('p'); progress.className = 'stream-progress'; progress.textContent = '현재 이야기를 이어 쓰는 중입니다…'; root.append(progress);
  }
}
...
function renderState(context, latest) {
  ...
  $('scene-state').replaceChildren(...[
    ['장소', scene.location_id || '확인 중'],
    ['현재 인물', (scene.present_npc_ids ?? []).join(', ') || '아직 없음']
  ].flatMap(([termText, valueText]) => {
    const term = document.createElement('dt'); term.textContent = termText;
    const value = document.createElement('dd'); value.textContent = valueText; return [term, value];
  }));
  $('player-situation').replaceChildren(...[
    ['이름', player.name || '플레이어'], ['경험치', String(player.exp ?? 0)], ['상태', '자유 입력으로 다음 행동을 정합니다.']
  ].flatMap(([termText, valueText]) => {
    const term = document.createElement('dt'); term.textContent = termText;
    const value = document.createElement('dd'); value.textContent = valueText; return [term, value];
  }));
  const monitor = latest?.mind_monitor ?? {};
  $('mind-monitor').replaceChildren(...Object.entries(monitor).map(([id, value]) => {
    const card = document.createElement('article'); card.className = 'mind-card';
    const title = document.createElement('h3'); title.textContent = id;
    const surface = document.createElement('p'); surface.textContent = value?.surface ?? '';
    const subconscious = document.createElement('p'); subconscious.textContent = value?.subconscious ?? '';
    card.append(title, surface, subconscious); return card;
  }));
...
function formatClock(minutes) {
  const value = Number.isInteger(minutes) ? minutes : 540;
  return `${String(Math.floor(value / 60)).padStart(2, '0')}:${String(value % 60).padStart(2, '0')}`;
}

async function reconnect(job) {
  setStatus('진행 중인 작업을 다시 확인하고 있습니다.');
  for (let attempt = 0; attempt < 120; attempt += 1) {
    const context = await api(`/api/v2/context?game_id=${encodeURIComponent(state.gameId)}`);
    render(context);
    if (context.job?.status !== 'processing') return context;
    await wait(250);
  }
  setStatus('서버 작업이 아직 진행 중입니다. 잠시 후 다시 확인해 주세요.', true);
  return null;
}
...
async function submit() {
  ...
  setStatus('이야기를 생성하고 저장하는 중입니다.');
  try {
    const response = await request('/api/v2/turn', { method: 'POST', body: JSON.stringify({ game_id: state.gameId, action_id: state.actionId, expected_turn: state.expectedTurn, retry_failed: state.retryFailed, literal_action: literalAction }) });
    if (response.headers.get('content-type')?.includes('text/event-stream')) {
      const terminal = await readStream(response);
      if (!terminal) await reconnect({});
    } else {
      const payload = await response.json();
      if (!payload.ok) throw new Error(payload.error?.message ?? payload.error?.code ?? '턴이 실패했습니다.');
      render(payload.data.context ?? await api(`/api/v2/context?game_id=${encodeURIComponent(state.gameId)}`));
      if (payload.data.job?.status === 'processing') await reconnect(payload.data.job);
    }
  } catch (error) { setStatus(error.message, true); }
  finally { $('submit-action').disabled = false; }
}
...
async function readStream(response) {
  ...
  while (true) {
    ...
    for (const event of events) {
      ...
      if (event.startsWith('event: story_delta')) $('current-story').textContent += data.text;
      if (event.startsWith('event: terminal')) {
        terminal = true;
        if (data.status === 'committed') { render(data.context); $('player-action').value = ''; setStatus('저장되었습니다.'); }
        if (data.status === 'failed') { render(data.context ?? await api(`/api/v2/context?game_id=${encodeURIComponent(state.gameId)}`)); setStatus(`턴 실패: ${data.error_code ?? 'turn_failed'}. 같은 행동을 명시적으로 다시 시도할 수 있습니다.`, true); }
      }
    }
  }
  return terminal;
