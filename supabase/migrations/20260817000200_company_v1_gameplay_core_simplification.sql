-- Company v1 gameplay-core simplification (source only; do not apply in Cut 1).
-- Fresh setup accepts literal time/location/character facts only. Historical
-- migrations and persisted rows remain readable at their compatibility boundary.
...
-- Fresh canonical scene validation is structural and intentionally narrow.
create or replace function public.company_validate_scene_v1(p_save jsonb, p_allow_legacy boolean default true)
returns jsonb
language plpgsql
immutable
as $$
