-- Company v1: restore structured_action into the game_turns INSERT at commit.
--
-- BACKGROUND
--   game_actions.structured_action is captured at reserve time (app transactions),
--   but the turn_guard migration (20260807000100) recreated commit_company_turn with
--   an INSERT column list that dropped structured_action, so every committed turn —
--   including genuine app transactions — records game_turns.structured_action = NULL.
--
-- FIX (apply-only migration, NOT yet applied to any Supabase project)
--   Recreate commit_company_turn to copy v_action.structured_action into the
--   game_turns INSERT. No new RPC argument is introduced: the value already lives
--   on game_actions and is simply copied at commit time.
--
-- VERIFICATION
--   - ordinary free-text turns: game_turns.structured_action stays NULL (normal)
--   - app transaction turns: game_turns.structured_action equals game_actions.structured_action

drop function if exists public.commit_company_turn(uuid, uuid, integer, jsonb, text, jsonb, jsonb);
